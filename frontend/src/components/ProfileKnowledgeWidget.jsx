...
const ProfileKnowledgeWidget = (props) => {
  const isNonMobileScreen = useMediaQuery("(min-width: 1000px)");
  const { globalReload } = useGlobalState()
  const [isLoading, setIsLoading] = React.useState(true)

  const [ knowledgeData, setKnowledgeData ] = React.useState(null);

  React.useEffect(() => {
    async function getData() {
      try {
        const response = await apiCall('GET', `/${props.userType}/profile/knowledge?zID=${props.zID}`)
        // const response = {}
        if (!response.error) {
          setKnowledgeData(response.knowledge)
          setIsLoading(false)
        }
        return response
      } catch(e) {
  ...
  return (
    <WidgetWrapper>
      ...
      </Box>
      {
        ...
        <>
        {
          Object.keys(knowledgeData).length > 0
          &&
          <>
          <Divider />
...
export default ProfileKnowledgeWidget;
